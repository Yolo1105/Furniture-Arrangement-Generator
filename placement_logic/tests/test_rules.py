...
# Try to import the real components, with fallbacks
try:
    ...
    class Furniture:
        def __init__(self, id, type, x, y, width, height, rotation=0):
            self.id = id
            self.type = type
            self.x = x
            self.y = y
...
try:
    ...
    # Mock generation function that returns a simple layout
    def generate_layout(room=None):
        """Mock layout generator for testing"""
        return [
            Furniture(id="bed_1", type=FurnitureType.BED, x=2, y=5, width=2, height=1.5, rotation=0),
            Furniture(id="table_1", type=FurnitureType.TABLE, x=6, y=5, width=1.2, height=0.8, rotation=0),
            Furniture(id="chair_1", type=FurnitureType.CHAIR, x=6.5, y=4, width=0.5, height=0.5, rotation=0)
        ]
